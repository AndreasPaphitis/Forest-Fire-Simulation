...
try:
    from src.utils.terrain_preprocessor_rasterio import TerrainPreprocessingConfig, TerrainPreprocessorRasterioFixed
    HAS_TERRAIN_PREPROCESSING = True
except ImportError:
    HAS_TERRAIN_PREPROCESSING = False
    TerrainPreprocessingConfig = None
    TerrainPreprocessorRasterioFixed = None
...
class BaseForestModel(ABC):
    ...
    def _load_dense_preprocessed_terrain_data(self, preprocessed_dir: str) -> bool:
        ...
        # TEMPORARY FIX: Bypass import check to allow terrain loading
        # if not HAS_TERRAIN_PREPROCESSING:
        #     logger.error("Terrain preprocessing utilities not available")
        #     return False
        
        # Force terrain loading to work regardless of import status
        logger.info("🔄 Bypassing import check - loading terrain data directly")
...
class MemoryOptimizedForestModel(ForestModel):
    ...
    def _initialize_directly_as_sparse(self, grid_size, num_layers, layer_height_meters, 
        ...
        try:
            ...
            logger.error(f"❌ Critical error during sparse initialization: {e}")
            raise RuntimeError(f"Sparse initialization failed: {e}")
        ...
        # CRITICAL FIX: Add terrain loading for large grids
        if config and hasattr(config, 'use_preprocessed_terrain') and config.use_preprocessed_terrain:
            if hasattr(config, 'preprocessed_terrain_dir') and config.preprocessed_terrain_dir:
                logger.info(f"🏔️  Loading preprocessed terrain data for massive sparse grid")
                try:
                    terrain_success = self._load_preprocessed_terrain_data(config.preprocessed_terrain_dir)
                    if terrain_success:
                        logger.info(f"✅ Terrain data loaded successfully for massive grid")
                        # Verify terrain was loaded with actual elevation values
                        if hasattr(self, 'terrain_elevation') and self.terrain_elevation is not None:
                            elev_min = np.min(self.terrain_elevation)
                            elev_max = np.max(self.terrain_elevation)
                            elev_range = elev_max - elev_min
                            logger.info(f"   🏔️  Terrain elevation: {elev_min:.1f}m to {elev_max:.1f}m (range: {elev_range:.1f}m)")
                            
                            # Check if terrain is actually loaded (not flat)
                            if elev_range < 1.0:
                                logger.warning(f"   ⚠️  WARNING: Terrain appears flat (range < 1m) - subsetting may have failed")
                                logger.warning(f"   🔍 Terrain array shape: {self.terrain_elevation.shape}")
                            else:
                                logger.info(f"   ✅ Terrain loaded successfully with realistic elevation")
                        else:
                            logger.warning(f"⚠️  Failed to load terrain data for normal grid")
                    except Exception as e:
    ...
    def ensure_shared_terrain_usage(self):
        ...
        # For sparse models, load terrain data directly without going through load_terrain_data
