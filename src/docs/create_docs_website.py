...
def preprocess_markdown_files(docs_dir):
    """Preprocess markdown files to fix common issues with anchors and links."""
    print("Preprocessing markdown files to fix anchor issues...")
    
    # Map of sections that might be referenced with different anchor formats
    anchor_mappings = {
    ...
    # Read the technical reference file
    ...
    if os.path.exists(tech_ref_path):
        ...
        # Add anchor IDs to headings for cross-referencing
        for anchor_id, heading_id in anchor_mappings.items():
            anchor_id = anchor_id.replace("#", "")
            heading_id = heading_id.replace("#", "")
            
            # Find heading patterns and add IDs
            for heading_level in range(1, 4):  # h1, h2, h3
                heading_marker = "#" * heading_level
                
                # Example: ## 2.4 Method Interrelationships -> ## 2.4 Method Interrelationships {#24-method-interrelationships}
                # First try with section numbers
                pattern = f"{heading_marker} \\d+\\.\\d+ [A-Za-z ]+\\b"
                matches = re.findall(pattern, content)
                for match in matches:
                    if any(keyword in match.lower() for keyword in heading_id.lower().split("-")):
                        new_heading = f"{match} {{#{anchor_id}}}"
                        content = content.replace(match, new_heading)
                
                # Then try without section numbers
                pattern = f"{heading_marker} [A-Za-z ]+\\b"
                matches = re.findall(pattern, content)
                for match in matches:
                    if any(keyword in match.lower() for keyword in heading_id.lower().split("-")):
                        if "{#" not in match:  # Only add if not already has an ID
                            new_heading = f"{match} {{#{anchor_id}}}"
                            content = content.replace(match, new_heading)
    ...
    # Now fix links in the documentation file
    doc_path = os.path.join(docs_dir, "Forest_Fire_Simulation_Documentation.md")
    if os.path.exists(doc_path):
        with open(doc_path, "r", encoding="utf-8") as f:
            content = f.read()
        
        # Add sections for missing anchors in the same file
        missing_anchors = [
        ...
        # Add hidden anchor tags at the end of the file for missing sections
        anchor_section = "\n\n<!-- Hidden anchors for cross-references -->\n"
        for anchor in missing_anchors:
            anchor_name = anchor.replace("#", "")
            anchor_section += f"<div id=\"{anchor_name}\" style=\"display:none\"></div>\n"
        
        content += anchor_section
...
def post_process_html_files(site_dir):
    ...
    # Map of anchors to fix in HTML files
    anchor_mappings = {
        "24-method-interrelationships": "method-interrelationships",
        "64-api-reference": "api-reference",
        "310-environmental-data-integration": "environmental-data-integration",
    ...
    # Process the technical reference HTML
    tech_ref_html = os.path.join(site_dir, "Forest_Fire_Simulation_Technical_Reference", "index.html")
    if os.path.exists(tech_ref_html):
        with open(tech_ref_html, "r", encoding="utf-8") as f:
            content = f.read()
        
        # Add anchor divs for each mapping
        anchor_divs = ""
        for anchor_id, heading_id in anchor_mappings.items():
            anchor_divs += f'<div id="{anchor_id}" class="hidden-anchor"></div>\n'
        
        # Insert before closing body tag
        content = content.replace("</body>", anchor_divs + "</body>")
        
        # Add CSS for hidden anchors
        css = """
<style>
.hidden-anchor {
    display: none;
...
"""
        ...
        with open(tech_ref_html, "w", encoding="utf-8") as f:
            f.write(content)
        
        print("Added hidden anchor divs to technical reference HTML")
    
    # Process the documentation HTML to add missing anchors
    doc_html = os.path.join(site_dir, "Forest_Fire_Simulation_Documentation", "index.html")
    if os.path.exists(doc_html):
        with open(doc_html, "r", encoding="utf-8") as f:
            content = f.read()
        
        # Add anchor divs for missing sections
        missing_anchors = [
            "42-plant-area-density-pad-and-forest-structure",
            "102-recovery-mechanisms",
            "103-health-monitoring",
        ...
        # Add CSS for hidden anchors if not already present
        if '.hidden-anchor' not in content:
            css = """
<style>
.hidden-anchor {
    display: none;
