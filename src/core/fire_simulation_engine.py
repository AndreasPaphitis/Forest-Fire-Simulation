...
# Tile-level parallelization settings
...
# Supertile-level parallelization settings (for very large landscapes)
USE_SUPERTILE_PARALLEL = True     # Whether to use supertile-level parallelization for large landscapes
SUPERTILE_SIZE = 500              # Size of each supertile in grid cells
SUPERTILE_OVERLAP = 20            # Overlap between adjacent supertiles
SUPERTILE_SIZE_THRESHOLD = 1000   # Grid size threshold for using supertile parallelization
...
class ForestModel:
    ...
    def load_fuel_moisture(self, moisture_file_path, default_moisture=0.3, layer_specific=True, no_data_value=-9999, moisture_range=(0.1, 0.9)):
        ...
        Args:
            moisture_file_path (str): Path to the moisture raster file
            default_moisture (float): Default moisture value for cells without data
        ...
        """
        try:
            print(f"Loading fuel moisture data from: {moisture_file_path}")
            
            # Initialize fuel moisture array
            ...
            # Open the raster file
            moisture_raster = gdal.Open(moisture_file_path)
            if moisture_raster is None:
                print(f"Error: Could not open {moisture_file_path}")
                return False
                
            # Get raster dimensions
            raster_width = moisture_raster.RasterXSize
            raster_height = moisture_raster.RasterYSize
            
            # Read the raster band
            band = moisture_raster.GetRasterBand(1)
            moisture_data = band.ReadAsArray()
            
            # Replace no_data_value with default moisture
            moisture_data = np.where(moisture_data == no_data_value, default_moisture, moisture_data)
            
            # Normalize moisture values to the specified range if needed
            if moisture_range:
                min_val, max_val = np.min(moisture_data), np.max(moisture_data)
                if min_val != max_val:  # Avoid division by zero
                    moisture_data = moisture_range[0] + (moisture_data - min_val) * (moisture_range[1] - moisture_range[0]) / (max_val - min_val)
                else:
                    moisture_data.fill(default_moisture)
            
            # Check if resampling is needed
            if moisture_data.shape[0] != self.grid_size_x or moisture_data.shape[1] != self.grid_size_y:
                print(f"Resampling moisture data from {moisture_data.shape} to match forest grid size {self.grid_size_x}x{self.grid_size_y}")
                
                # Resize using bilinear interpolation
                from scipy.ndimage import zoom
                zoom_x = self.grid_size_x / moisture_data.shape[0]
                zoom_y = self.grid_size_y / moisture_data.shape[1]
                moisture_data = zoom(moisture_data, (zoom_x, zoom_y), order=1, mode='nearest')
            
            # Apply layer-specific moisture values if specified and available
            if layer_specific and moisture_raster.RasterCount > 1:
                ...
                for z in range(min(self.num_layers, moisture_raster.RasterCount)):
                    ...
                    # Replace no_data_value with default moisture
                    layer_data = np.where(layer_data == no_data_value, default_moisture, layer_data)
                    
                    # Normalize moisture values if needed
                    if moisture_range:
                        min_val, max_val = np.min(layer_data), np.max(layer_data)
                        if min_val != max_val:
                            layer_data = moisture_range[0] + (layer_data - min_val) * (moisture_range[1] - moisture_range[0]) / (max_val - min_val)
                        else:
                            layer_data.fill(default_moisture)
                    
                    # Resize if needed
                    if layer_data.shape[0] != self.grid_size_x or layer_data.shape[1] != self.grid_size_y:
                        from scipy.ndimage import zoom
                        zoom_x = self.grid_size_x / layer_data.shape[0]
                        zoom_y = self.grid_size_y / layer_data.shape[1]
                        layer_data = zoom(layer_data, (zoom_x, zoom_y), order=1, mode='nearest')
            ...
            min_moisture = np.min(self.fuel_moisture)
            max_moisture = np.max(self.fuel_moisture)
            mean_moisture = np.mean(self.fuel_moisture)
            
            print(f"Fuel moisture data loaded successfully:")
            print(f"  Dimensions: {self.grid_size_x}x{self.grid_size_y}x{self.num_layers}")
            print(f"  Moisture range: {min_moisture:.3f} - {max_moisture:.3f}")
            print(f"  Mean moisture: {mean_moisture:.3f}")
            
            return True
            
        except Exception as e:
            print(f"Error loading fuel moisture data: {str(e)}")
            traceback.print_exc()
            self.fuel_moisture = np.full((self.grid_size_x, self.grid_size_y, self.num_layers), default_moisture, dtype=np.float32)
            return False
...
def main():
    ...
    USE_PARALLEL_PROCESSING = True  # Whether to use parallel processing for tile operations
    NUM_PARALLEL_WORKERS = None  # Number of parallel workers (None = use all available cores)
    ...
    if LIDAR_AREA_NAME:
        print(f"LiDAR area name: {LIDAR_AREA_NAME}")
    if SAVE_RESULTS:
        print(f"Results will be saved to: {RESULTS_DIR}")
    else:
        print("Results will not be saved to disk")
    
    # Initialize forest model
    forest_model = None
    
    # Initialize the forest model based on the fuel load method
    if FUEL_LOAD_METHOD == 'tiled_lidar':
        print("\n==== TILED LIDAR INTEGRATION MODE ====")
        
        # Initialize the forest model for the case where tiled_lidar is not available
        forest_model_fallback = None
        
        try:
            # Check if the vegetation data integration.py file exists
            if not os.path.exists('vegetation_data_integration.py'):
                raise ImportError("vegetation_data_integration.py file not found in the current directory")
            
            print(f"Found vegetation_data_integration.py file: {os.path.exists('vegetation_data_integration.py')}")
            
            # Import the module and class
            import vegetation_data_integration
            from vegetation_data_integration import TiledLiDARIntegration
            
            print("Successfully imported TiledLiDARIntegration module")
            
            # Create the forest model directly from all rasters in all datasets
            print("\nInitializing tiled LiDAR-based forest model...")
            print(f"Base directory: {LIDAR_RASTER_BASE_DIR}")
            
            try:
                # MULTI-DATASET APPROACH:
                # Create the forest model from all available rasters
                forest_model = TiledLiDARIntegration.create_model_from_rasters(
                    base_dir=LIDAR_RASTER_BASE_DIR,
                    num_layers=NUM_LAYERS,  # Use specified number of layers
                    layer_height_meters=LAYER_HEIGHT_METERS,
                    target_resolution=MODEL_RESOLUTION,  # Use the global resolution parameter
                    tile_size=TILED_LIDAR_TILE_SIZE,     # Use the defined tile size constant
                    overlap=TILED_LIDAR_OVERLAP          # Use the defined overlap constant
                )
                
                print(f"Successfully created forest model from tiled LiDAR data")
                print(f"Model has {forest_model.grid_size}x{forest_model.grid_size} cells")
                print(f"Model has {forest_model.num_layers} vertical layers")
                
            except Exception as e:
                # If there's an error initializing from the tiled approach
                print(f"Error initializing tiled LiDAR model: {str(e)}")
                print("Stack trace:")
                traceback.print_exc()
                print("\nFalling back to standard initialization...")
                
                # Create a fallback forest model using standard initialization
                if USE_MEMORY_OPTIMIZATIONS:
                    ...
                    forest_model = ForestModel(grid_size=GRID_SIZE, num_layers=NUM_LAYERS,
                                             layer_height_meters=LAYER_HEIGHT_METERS)
                FUEL_LOAD_METHOD = 'random'  # Fall back to random
        
        except ImportError as e:
            # If the TiledLiDARIntegration module can't be imported
            print(f"Failed to import TiledLiDARIntegration: {str(e)}")
        ...
        if USE_MEMORY_OPTIMIZATIONS:
            ...
            # Standard forest model
            forest_model = ForestModel(GRID_SIZE, NUM_LAYERS, LAYER_HEIGHT_METERS)
    ...
    if IGNITION_METHOD == 'center':
        ...
        for x, y, z in sampled_cells:
            forest_model.set_ignition(x, y, z)
    elif IGNITION_METHOD == 'corner':
        # Single ignition point at corner of the grid
        forest_model.set_ignition(0, 0, 0)
    elif IGNITION_METHOD == 'multiple':
        # Multiple ignition points at specified locations
        if IGNITION_COORDINATES:
            for x, y, z in IGNITION_COORDINATES:
                forest_model.set_ignition(x, y, z)
        else:
            # Default to random ignition if no coordinates provided
            for _ in range(NUM_IGNITION_POINTS):
                x = random.randint(0, forest_model.grid_size - 1)
                y = random.randint(0, forest_model.grid_size - 1)
                z = 0  # Bottom layer
                forest_model.set_ignition(x, y, z)
    elif IGNITION_METHOD == 'coordinates':
        # Ignition at specific coordinates
        if IGNITION_COORDINATES:
            for x, y, z in IGNITION_COORDINATES:
                forest_model.set_ignition(x, y, z)
        else:
            # Default to center ignition if no coordinates provided
            center = forest_model.grid_size // 2
            forest_model.set_ignition(center, center, 0)
    ...
    # Clean up resources for optimized models
    if USE_MEMORY_OPTIMIZATIONS and hasattr(forest_model, 'clean_up'):
        forest_model.clean_up()
...
class MemoryOptimizedForestModel(ForestModel):
    ...
    def __init__(self, grid_size=100, num_layers=10, layer_height_meters=LAYER_HEIGHT_METERS,
        ...
        Args:
            ...
            use_parallel: Whether to use parallel processing for tile operations
            num_workers: Number of parallel workers (None = use all available cores)
        """
    ...
    def update_horizontal_spread_parallel(self, num_workers=None):
        ...
        Args:
            num_workers: Number of parallel workers to use. If None,
                        defaults to the number of CPU cores.
                        
        Returns:
            bool: True if any cells are still burning after this step
        """
        # Set up number of workers
        if num_workers is None:
            num_workers = self.num_workers
        
        # Determine optimal tile size - we want roughly 2-4x as many tiles as workers
        # for good load balancing, but not too small to minimize overhead
        tile_size_x = min(200, max(50, self.grid_size_x // (2 * num_workers)))
        tile_size_y = min(200, max(50, self.grid_size_y // (2 * num_workers)))
        
        # Overlap is needed to handle boundary conditions correctly
        overlap = 2
        
        # Calculate number of tiles
        num_tiles_x = (self.grid_size_x + tile_size_x - 1) // tile_size_x
        num_tiles_y = (self.grid_size_y + tile_size_y - 1) // tile_size_y
        ...
        # Fire spread parameters needed by each worker
        shared_params = {
            'fire_params': self.fire_params if hasattr(self, 'fire_params') else {
                'k': 1.5,            # Steepness of sigmoid curve
                'threshold': 1.0,    # Critical number of burning neighbors
                'spread_threshold': 0.01  # Minimum probability to consider ignition
            },
            'fuel_consumption_rates': self.fuel_consumption_rates,
            'WIND_EFFECT_MAX': WIND_EFFECT_MAX,
        ...
        for step in progress_bar:
            ...
            # Update fire spread (horizontal first, then vertical)
    ...
    def _process_tile_parallel(self, tile_data):
        """
        Process a single tile for parallel fire spread calculation.
        
        This method is designed to be called by parallel workers and handles
        the fire spread calculation for a specific portion of the forest grid.
        
        Args:
            tile_data: Dictionary containing tile information:
                - start_x, start_y: Starting coordinates of tile
                - end_x, end_y: Ending coordinates of tile
                - layers: List of layer data for this tile
                - fuel_load: Fuel load data for this tile
                - wind_u, wind_v: Wind components for this tile
                - fuel_moisture: Fuel moisture for this tile
                
        Returns:
            Dictionary with processed tile data and changes
        """
        # Extract tile information
        start_x = tile_data['start_x']
        start_y = tile_data['start_y']
        end_x = tile_data['end_x']
        end_y = tile_data['end_y']
        layers = tile_data['layers']
        fuel_load = tile_data['fuel_load']
        wind_u = tile_data['wind_u']
        wind_v = tile_data['wind_v']
        fuel_moisture = tile_data['fuel_moisture']
        fire_params = tile_data['fire_params']
        
        # Create copies of layers to track changes independently
        new_layers = [layer.copy() for layer in layers]
        ...
        for z in range(len(layers)):
            # Skip layers with no burning cells
            if not np.any(layers[z] == CellState.BURNING.value):
                continue
                
            # Process each cell in the layer
            height, width = layers[z].shape
            for y in range(1, height - 1):
                for x in range(1, width - 1):
                    # Skip cells that are not burning
                    if layers[z][y, x] != CellState.BURNING.value:
                        continue
                        
                    # Consuming fuel logic
                    fuel_load[z, y, x] -= tile_data['fuel_consumption_rates'][z]
                    
                    # If fuel is depleted, mark as burned
                    if fuel_load[z, y, x] <= 0:
                        new_layers[z][y, x] = CellState.BURNED.value
                        fuel_load[z, y, x] = 0
                        continue
                    ...
                    for ny, nx in neighbors:
                        ...
                        # Skip if neighbor is not unburned
                        if layers[z][ny, nx] != CellState.UNBURNED.value:
                            continue
                            
                        # Skip if neighbor has no fuel
                        if fuel_load[z, ny, nx] <= 0:
                            continue
                        ...
                        for dy in [-1, 0, 1]:
                            for dx in [-1, 0, 1]:
                                ...
                                # Check if within bounds
                                if (0 <= neighbor_y < height and 
                                    0 <= neighbor_x < width):
                                    if layers[z][neighbor_y, neighbor_x] == CellState.BURNING.value:
                                        n_burning += 1
                        ...
                        wind_effect = wind_u[y, x] * dx + wind_v[y, x] * dy
                        wind_effect = max(0, wind_effect) * tile_data['WIND_EFFECT_MAX']
                        
                        # Calculate ignition probability
                        base_prob = sigmoid_ignition_neighbors(
                            n_burning,
                            fire_params['k'],
                            fire_params['threshold']
                        )
                        
                        # Apply modifiers
                        fuel_effect = fuel_load[z, ny, nx]
                        ignition_prob = base_prob * fuel_effect * (1 + wind_effect)
                        ignition_prob *= (1 - fuel_moisture)
                        
                        # Apply distance penalty for diagonal neighbors
                        if abs(nx - x) + abs(ny - y) > 1:  # Diagonal neighbor
                            ignition_prob *= tile_data['DIAGONAL_PENALTY']
                        
                        # Check if ignition occurs
                        if (ignition_prob > fire_params['spread_threshold'] and 
                            random.random() < ignition_prob):
                            new_layers[z][ny, nx] = CellState.BURNING.value
                            any_burning = True
        
        # Return processed tile with updated layers and fuel load
        return {
            'start_x': start_x,
            'start_y': start_y,
            'end_x': end_x,
    ...
    def run_simulation(self, max_steps=MAX_STEPS, stop_when_fire_extinguished=STOP_WHEN_FIRE_EXTINGUISHED, use_parallel=True, num_workers=None):
        ...
        Args:
            ...
            num_workers: Number of parallel workers to use (None = use class default)
        ...
        use_parallel_local = use_parallel and self.use_parallel
        if num_workers is None:
            num_workers = self.num_workers
        
        # Log parallel processing information
        if use_parallel_local:
        ...
        for step in progress_bar:
            ...
            if use_parallel_local:
                ...
                any_burning = self.update_horizontal_spread_parallel(num_workers)
            else:
    ...
    def process_supertiles_parallel(self, supertiles, num_workers=None):
        """
        Process multiple supertiles in parallel.
        
        This method enables parallel processing across supertiles, providing another
        level of parallelism beyond the tile-level parallelization. This is especially
        useful for very large landscapes that are divided into multiple supertiles.
        
        Args:
            supertiles: List of supertile data dictionaries
            num_workers: Number of worker processes (None = use all available cores)
        
        Returns:
            List of processed supertiles
        """
        # Determine number of workers
        if num_workers is None:
            num_workers = multiprocessing.cpu_count()
        
        logger.info(f"Processing {len(supertiles)} supertiles in parallel with {num_workers} workers")
        
        # Create a shallow copy of each supertile to avoid modifying the original data
        supertile_copies = [supertile.copy() for supertile in supertiles]
        
        # Process supertiles in parallel
        processed_supertiles = []
        
        with ProcessPoolExecutor(max_workers=num_workers) as executor:
            # Submit all supertiles for processing
            future_to_supertile = {
                executor.submit(self._process_single_supertile, supertile): i 
                for i, supertile in enumerate(supertile_copies)
            }
            
            # Process completed futures as they finish
            for future in as_completed(future_to_supertile):
                supertile_idx = future_to_supertile[future]
                try:
                    processed_supertile = future.result()
                    processed_supertiles.append(processed_supertile)
                    logger.info(f"Completed processing supertile {supertile_idx}")
                except Exception as e:
                    logger.error(f"Error processing supertile {supertile_idx}: {str(e)}")
                    traceback.print_exc()
        
        return processed_supertiles

    def _process_single_supertile(self, supertile):
        """
        Process a single supertile, with internal parallelization at the tile level.
        
        This method is designed to be called by parallel workers from process_supertiles_parallel.
        It handles the complete simulation for a specific supertile, including both horizontal
        and vertical fire spread, using tile-level parallelization internally.
        
        Args:
            supertile: Dictionary containing supertile information
        
        Returns:
            Processed supertile data
        """
        # Extract supertile data
        bounds = supertile['bounds']
        state = supertile['state'].copy()
        fuel_load = supertile['fuel_load'].copy()
        
        # Create a mini-model for this supertile
        # This allows us to use the existing tile-level parallelization
        mini_model = MemoryOptimizedForestModel(
            grid_size=(bounds[3] - bounds[0], bounds[4] - bounds[1]),
            num_layers=bounds[5] - bounds[2] + 1,
            layer_height_meters=self.layer_height_meters,
            memory_optimization_level=0,  # No need for memory optimization in the mini-model
            use_parallel=True  # Enable internal parallelization
        )
        
        # Initialize the mini-model with data from this supertile
        mini_model.state = state
        mini_model.fuel_load = fuel_load
        
        # Copy other relevant properties
        if 'wind_u' in supertile and 'wind_v' in supertile:
            mini_model.wind_u = supertile['wind_u'].copy()
            mini_model.wind_v = supertile['wind_v'].copy()
        
        mini_model.fuel_moisture = supertile.get('fuel_moisture', self.fuel_moisture)
        mini_model.fuel_consumption_rates = supertile.get('fuel_consumption_rates', self.fuel_consumption_rates)
        mini_model.fire_params = supertile.get('fire_params', self.fire_params)
        
        if hasattr(self, 'vertical_connectivity'):
            mini_model.vertical_connectivity = self.vertical_connectivity
        
        # Run simulation for this supertile for one step
        # We use the update methods directly to avoid overhead
        any_burning = mini_model.update_horizontal_spread_parallel()
        any_burning_vertical = mini_model.update_vertical_spread()
        
        # Update the supertile data with the results
        supertile['state'] = mini_model.state
        supertile['fuel_load'] = mini_model.fuel_load
        supertile['any_burning'] = any_burning or any_burning_vertical
        
        return supertile

    def generate_supertiles(self, supertile_size=500, overlap=10):
        """
        Divide the large landscape into supertiles for parallel processing.
        
        This method creates a set of overlapping supertiles that cover the entire
        landscape. Each supertile contains a portion of the grid state and all
        necessary data to simulate fire spread independently.
        
        Args:
            supertile_size: Size of each supertile in grid cells
            overlap: Overlap between adjacent supertiles
        
        Returns:
            List of supertile dictionaries
        """
        grid_size_x, grid_size_y = self.grid_size_x, self.grid_size_y
        num_layers = self.num_layers
        
        # Check if supertile division is needed
        if grid_size_x <= supertile_size and grid_size_y <= supertile_size:
            logger.info("Grid size is smaller than supertile size, no supertile division needed")
            return None
        
        logger.info(f"Dividing landscape into supertiles (size={supertile_size}, overlap={overlap})")
        
        # Calculate number of supertiles in each dimension
        num_supertiles_x = max(1, (grid_size_x + supertile_size - 1 - overlap) // (supertile_size - overlap))
        num_supertiles_y = max(1, (grid_size_y + supertile_size - 1 - overlap) // (supertile_size - overlap))
        
        logger.info(f"Creating {num_supertiles_x}x{num_supertiles_y} supertile grid")
        
        # Create supertiles
        supertiles = []
        
        for sy in range(num_supertiles_y):
            for sx in range(num_supertiles_x):
                # Calculate supertile bounds with overlap
                min_x = max(0, sx * (supertile_size - overlap) - overlap)
                min_y = max(0, sy * (supertile_size - overlap) - overlap)
                
                max_x = min(grid_size_x, min_x + supertile_size)
                max_y = min(grid_size_y, min_y + supertile_size)
                
                # Create supertile with full vertical extent
                min_z = 0
                max_z = num_layers - 1
                
                # Extract supertile state and data
                supertile_state = self.state[min_x:max_x, min_y:max_y, min_z:max_z+1].copy()
                supertile_fuel_load = self.fuel_load[:, min_y:max_y, min_x:max_x].copy()
                
                # Extract wind data if available
                supertile_wind_u = None
                supertile_wind_v = None
                if hasattr(self, 'wind_u') and hasattr(self, 'wind_v'):
                    supertile_wind_u = self.wind_u[min_y:max_y, min_x:max_x].copy()
                    supertile_wind_v = self.wind_v[min_y:max_y, min_x:max_x].copy()
                
                # Create supertile dictionary
                supertile = {
                    'bounds': (min_x, min_y, min_z, max_x, max_y, max_z),
                    'state': supertile_state,
                    'fuel_load': supertile_fuel_load,
                    'wind_u': supertile_wind_u,
                    'wind_v': supertile_wind_v,
                    'fuel_moisture': self.fuel_moisture,
                    'fuel_consumption_rates': self.fuel_consumption_rates.copy(),
                    'fire_params': self.fire_params.copy(),
                    'grid_coords': (sx, sy)  # Store grid coordinates for debugging
                }
                
                # Add vertical connectivity if available
                if hasattr(self, 'vertical_connectivity'):
                    supertile['vertical_connectivity'] = self.vertical_connectivity.copy()
                
                supertiles.append(supertile)
        
        logger.info(f"Created {len(supertiles)} supertiles")
        return supertiles

    def update_from_supertiles(self, supertiles):
        """
        Update the main landscape state from processed supertiles.
        
        This method merges the results from all supertiles back into the main
        landscape state, handling overlapping regions appropriately.
        
        Args:
            supertiles: List of processed supertile dictionaries
        
        Returns:
            bool: True if any cells are still burning
        """
        # Create new state to avoid conflicts
        new_state = self.state.copy()
        
        # Track if any burning is occurring
        any_burning = False
        
        # Process each supertile
        for supertile in supertiles:
            min_x, min_y, min_z, max_x, max_y, max_z = supertile['bounds']
            supertile_state = supertile['state']
            
            # Calculate the shape of this supertile
            shape_x = max_x - min_x
            shape_y = max_y - min_y
            shape_z = max_z - min_z + 1
            
            # Sanity check the shapes
            if supertile_state.shape != (shape_x, shape_y, shape_z):
                logger.warning(f"Supertile shape mismatch: expected {(shape_x, shape_y, shape_z)}, got {supertile_state.shape}")
                continue
            
            # Check if any burning occurred in this supertile
            if supertile.get('any_burning', False):
                any_burning = True
            
            # Merge state - priority: BURNING > BURNED > UNBURNED
            for z in range(shape_z):
                # Get slices for this layer
                main_slice = new_state[min_x:max_x, min_y:max_y, min_z+z]
                supertile_slice = supertile_state[:, :, z]
                
                # Update burning cells
                burning_mask = (supertile_slice == CellState.BURNING.value)
                if np.any(burning_mask):
                    main_slice[burning_mask] = CellState.BURNING.value
                
                # Update burned cells (only if the cell is not already burning)
                burned_mask = (supertile_slice == CellState.BURNED.value) & (main_slice != CellState.BURNING.value)
                if np.any(burned_mask):
                    main_slice[burned_mask] = CellState.BURNED.value
            
            # Update fuel load
            self.fuel_load[:, min_y:max_y, min_x:max_x] = supertile['fuel_load']
        
        # Update the landscape state
        self.state = new_state
        
        # Check if any burning is occurring in the whole landscape
        if not any_burning:
            any_burning = np.any(self.state == CellState.BURNING.value)
        
        return any_burning

    def run_simulation(self, max_steps=MAX_STEPS, stop_when_fire_extinguished=STOP_WHEN_FIRE_EXTINGUISHED, use_parallel=True, num_workers=None):
        """
        Run the simulation with memory optimization and multi-level parallelization.
        This overrides the base ForestModel.run_simulation method.
        ...
        logger.info(f"Starting simulation with parallel processing: {use_parallel}")
        ...
        # Determine if we should use supertile parallelization for large landscapes
        # Check configuration parameters and grid size
        grid_size = max(self.grid_size_x, self.grid_size_y)
        use_supertiles = (USE_SUPERTILE_PARALLEL and use_parallel and 
                           grid_size >= SUPERTILE_SIZE_THRESHOLD)
        
        if use_supertiles:
            logger.info(f"Using supertile-level parallelization for large landscape (grid size: {grid_size})")
            supertiles = self.generate_supertiles(supertile_size=SUPERTILE_SIZE, 
                                                  overlap=SUPERTILE_OVERLAP)
        else:
            supertiles = None
        ...
        for step in progress_bar:
            ...
            # Update fire spread using appropriate parallelization strategy
            if use_supertiles and supertiles:
                # Use supertile-level parallelization for very large landscapes
                logger.info(f"Step {step}: Processing {len(supertiles)} supertiles in parallel")
                processed_supertiles = self.process_supertiles_parallel(supertiles, num_workers)
                any_burning = self.update_from_supertiles(processed_supertiles)
                
                # Regenerate supertiles for the next step if needed
                if any_burning:
                    supertiles = self.generate_supertiles(supertile_size=SUPERTILE_SIZE, 
                                                          overlap=SUPERTILE_OVERLAP)
                
            elif use_parallel:
                # Use tile-level parallelization for regular landscapes
                any_burning = self.update_horizontal_spread_parallel(num_workers)
        ...
        # Record parallelization information in results
