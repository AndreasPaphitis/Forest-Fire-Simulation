...
class LiDARDataManager:
    ...
    def _find_layer_files(self, base_dir: Union[str, Path], layer: int) -> List[Path]:
        ...
        # Different patterns to match layer-specific files
        patterns = [
            # Format: pad_rasters/*_pad_{layer}.0m.tif
            f"**/*_pad_{layer}.0m.tif",
            # Format: pad_rasters/*_pad_{layer}m.tif
            f"**/*_pad_{layer}m.tif",
            # Format: pad/*_layer_{layer}.tif
            f"**/pad/*_layer_{layer}.tif",
            # Format: pad/layer_{layer}/*.tif
            f"**/pad/layer_{layer}/*.tif"
        ]
        
        # Find files matching any of the patterns
        layer_files = []
        for pattern in patterns:
            layer_files.extend(list(base_path.glob(pattern)))
        
        logger.info(f"Found {len(layer_files)} files for layer {layer} in {base_path}")
